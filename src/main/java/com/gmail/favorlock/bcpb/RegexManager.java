...
public class RegexManager {
	...
	public RegexManager(BungeeChatPlusBukkit plugin) {
		this.plugin = plugin;
	...
	public void loadRules() {
    	...
    	try {
    		...
    		plugin.getLogger().warning("Error reading config file '" + file + "': " + e.getLocalizedMessage());
    	}
    	catch (Exception e) {
    		e.printStackTrace();
    	}
    ...
    private void compilePattern(String re) {
    	...
    	if (patterns.get(re) == null) {
    		try {
    			...
    			plugin.getLogger().fine("Successfully compiled regex: " + re);
    		}
    		catch (PatternSyntaxException e) {
    			plugin.getLogger().warning("Failed to compile regex: " + re);
    			plugin.getLogger().warning(e.getMessage());
    		}
    		catch (Exception e) {
    			plugin.getLogger().severe("Unexpected error while compiling expression '" + re + "'");
    			e.printStackTrace();
    		}
    	}
    }
        ...
        if (!(player.hasPermission("BungeeChatPlus.bypass"))) {
	    	...
	    	for (String line : this.rules) {
	    		...
	    		if (matched) {  
		    		...
		    		if (valid == false) {
		    			plugin.getLogger().warning("Ignored syntax error in rules.txt: " + line);    			
		    		}	    		
	    		}
	    	}	
    ...
    public void logToFile(String message) {   
    	...
    	Boolean logEnabled = plugin.getBCPConfig().Settings_EnableRegexLog;
    	if (logEnabled) {	
	    	try {
	    		File directory = new File("plugins" + File.separator + plugin.getDescription().getName());
